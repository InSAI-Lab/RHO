...
def train(cfg: DictConfig, job_id: Optional[int] = None):
    ...
    # If gpus are specified in the config, they might represent the total number of GPUs.
    # The logic below for scaling batch size relies on `cfg.experiment.gpus` being the total count.
    # If it's not passed, we default to the detected total.
    if cfg.experiment.gpus is None:
        cfg.experiment.gpus = total_gpus
    ...
    if num_nodes > 1 and cfg.experiment.gpus != total_gpus:
        logger.warning(
            "Mismatch between specified GPUs (%d) and detected SLURM GPUs (%d nodes * %d gpus/node = %d). "
            "Using detected SLURM configuration.",
            cfg.experiment.gpus, num_nodes, gpus_per_node, total_gpus
        )
        cfg.experiment.gpus = total_gpus
    ...
    if cfg.experiment.gpus > 1:
        ...
        # The batch size and num_workers should be scaled by the total number of GPUs
        effective_gpus = cfg.experiment.gpus
        for split in ["train", "val"]:
            cfg.data["loading"][split].batch_size = (
                cfg.data["loading"][split].batch_size // effective_gpus
            )
            cfg.data["loading"][split].num_workers = int(
                (cfg.data["loading"][split].num_workers + effective_gpus - 1)
                / effective_gpus
            )
...
@hydra.main(
    config_path=osp.join(osp.dirname(__file__), "conf"), config_name="orienternet_120fov"
)
