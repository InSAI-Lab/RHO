...
def main():
    parser = argparse.ArgumentParser(description='Visualize panorama predictions.')
    parser.add_argument('--num_pano', type=int, default=1, help='Number of panoramas to visualize')
    parser.add_argument('--data_dir', type=str, default='RHO_clean', help='Path to the dataset directory')
    parser.add_argument('--out_dir', type=str, default='viz_results', help='Path to the output directory')
    parser.add_argument('--experiment', type=str, default='RHO_clean', help='Experiment name or checkpoint path')
    args = parser.parse_args()

    conf = OC.load(Path(maploc.__file__).parent / 'conf/data/mapillary_pano.yaml')
    conf = OC.merge(conf, OC.create(yaml.full_load(f"""
    data_dir: "datasets/{args.data_dir}"
    loading:
    ...
    experiment = args.experiment  # find the best checkpoint
    ...
    num_panoramas_to_viz = args.num_pano
